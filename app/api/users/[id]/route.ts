...
export async function PATCH(
    request: Request,
    { params }: { params: { id: string } } // Ambil id dari URL
) {
    const id = params.id;
    ...
    try {
        ...
        if (updateData.full_name !== undefined) {
            ...
            profileUpdates.full_name = updateData.full_name === '' ? null : updateData.full_name;
        }
            ...
            if (error) {
                ...
                profileUpdateError = error;
                 ...
                 if (!updatedProfileData) {
                     ...
                     profileUpdateError = { message: `Profile not found for user ID ${id}` }; // Error custom
                 }
            }
        } else {
             ...
             const { data, error } = await supabase.from('profiles').select('*').eq('user_id', id).single();
              if (error) {
                 console.error('Supabase SELECT Error (profiles, after no updates):', error);
        ...
        if (updateData.email !== undefined && updateData.email !== '') { // Cek juga bukan string kosong
        ...
        if (Object.keys(authUpdates).length > 0) {
            ...
            const { data: authData, error: authError } = await supabase.auth.admin.updateUserById(id, authUpdates);
            ...
            console.log(`No auth data to update for user ${id}.`);
             ...
             if (authError) {
                  console.error('Supabase GET Error (auth.users, after no auth updates):', authError);
        ...
        if (profileUpdateError || authUpdateError) {
             ...
             if(profileUpdateError) errorMessage += ` Profile update error: ${profileUpdateError.message}`;
             if(authUpdateError) errorMessage += ` Auth update error: ${authUpdateError.message}`;
             ...
             if (
                (authUpdateError && authUpdateError.message.includes('duplicate key value')) ||
                (authUpdateError && authUpdateError.message.includes('Email already registered'))
             ) {
                  return NextResponse.json({ message: 'User with this email already exists', error: authUpdateError.message }, { status: 409 }); // Conflict
             }
             if (authUpdateError && authUpdateError.message.includes('Password should be at least')) {
                  return NextResponse.json({ message: authUpdateError.message, error: authUpdateError.message }, { status: 400 }); // Bad Request
             }
            ...
            return NextResponse.json({
                ...
                updatedAuth: updatedAuthData ? { id: updatedAuthData.id, email: updatedAuthData.email } : null,
            }, { status: 500 });
        }
...
export async function DELETE(
  ...
  { params }: { params: { id: string } } // Tetap gunakan id di sini karena nama folder [id]
) {
  // Ambil ID profil dari URL params
  const profileId = params.id; // Menggunakan nama parameter URL 'id' untuk mengambil nilai ID profil
  ...
  try {
      ...
      console.log(`Attempting to find user_id for profile ID: ${profileId}`);
      const { data: profileData, error: findProfileError } = await supabase
          .from('profiles')
          .select('user_id')
          .eq('id', profileId) // Cari berdasarkan ID profil
          .single(); // Ambil satu baris

      if (findProfileError || !profileData) {
          console.error('Supabase SELECT Error (profiles - find user_id):', findProfileError);
          let errorMessage = `Profile with ID ${profileId} not found.`;
           if (findProfileError) {
      ...
      console.log(`Found user_id ${idToDelete} for profile ID ${profileId}. Proceeding to delete user.`);
      ...
      const { data: deleteUserData, error: deleteUserError } = await supabase.auth.admin.deleteUser(idToDelete);
      ...
      if (deleteUserError) {
          ...
          let errorMessage = 'Failed to delete user.';
           ...
           if (deleteUserError.message.includes('User not found')) {
               ...
               return NextResponse.json({ message: errorMessage, error: deleteUserError.message }, { status: 404 }); // Not Found
           }
          return NextResponse.json({ message: errorMessage, error: deleteUserError.message }, { status: 500 }); // Internal Server Error
      }

      console.log(`User successfully deleted with ID: ${idToDelete}`, deleteUserData);

      // Jika penghapusan dari auth.users berhasil, asumsikan trigger
      // untuk profiles juga berhasil atau akan berjalan.
      // Mengembalikan respons sukses.
      return NextResponse.json({ message: `User with ID ${idToDelete} (profile ID ${profileId}) deleted successfully.` }, { status: 200 }); // OK
